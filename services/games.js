...
export default class GameService extends Service {
  ...
  async checkStock(filter, confirm) {
    try {
  ...
  async stockCount(cart) {
    ...
    cart.gameInfo.forEach(array => {
      ...
      const game = gameArray.find(value => value[1]._id == array.gameId)

      game[1].stock -= cart.gameInfo[0].amount
  ...
  async checkPaying(cart) {
    ...
    let checked = cart.gameInfo.map(Array => {
      const gameArray = Object.entries(gameList)

      const game = gameArray.filter(value => value[1]._id == Array.gameId)

      return game[0][1].stock >= cart.gameInfo[0].amount
